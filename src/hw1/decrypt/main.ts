import { decryptService } from "../../modules/hw1/decrypt/decrypt.service";
...
const $bruteForceOutputElement = document.getElementById("bruteforce-output") as HTMLTextAreaElement;
...
function decrypt(event: SubmitEvent) {
  ...
  const { input, key, strategy } = Object.fromEntries(formData) as unknown as DecryptInput;
  ...
  $decryptOutputElement.textContent = decryptService.decrypt(input, parseInt(key), strategy);
}
...
function bruteForce(event: SubmitEvent) {
  ...
  const form = event.target! as HTMLFormElement;
  const formData = new FormData(form);
  const { input, dictionaryFile, strategy } = Object.fromEntries(formData) as unknown as BruteForceInput;
  ...
  decryptService.bruteForceDecrypt(dictionaryUrl, input, strategy).subscribe((result) => {
